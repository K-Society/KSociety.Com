...
namespace KSociety.Com.Driver.S7
{
    public partial class Plc
    {
        ...
        private static void BuildHeaderPackage(System.IO.MemoryStream stream, int amount = 1)
        {
            ...
            stream.WriteByteArray(Int.ToByteArray((short)(19 + (12 * amount))));
            stream.WriteByteArray(new byte[] { 0x02, 0xf0, 0x80, 0x32, 0x01, 0x00, 0x00, 0x00, 0x00 });
            //data part size
            stream.WriteByteArray(Word.ToByteArray((ushort)(2 + (amount * 12))));
        ...
        private static void BuildReadDataRequestPackage(System.IO.MemoryStream stream, DataType dataType, int db, int startByteAdr, int count = 1)
        {
            ...
            switch (dataType)
            {
                case DataType.Timer:
                case DataType.Counter:
                    stream.WriteByteArray(Word.ToByteArray((ushort)(startByteAdr)));
                    break;
                default:
                    stream.WriteByteArray(Word.ToByteArray((ushort)((startByteAdr) * 8)));
                    break;
            }
        }
