...
namespace KSociety.Com.Driver.Enip.Eipnet.Cip
{
    public static class Ioi
    {
        ...
        private static List<IOISegment> SegmentConverter(string ioiAddress)
        {
            ...
            for (int i = 0; i < ioiAddress.Length; i++)
            {
                if (currentSegmentType == IOISegmentType.Symbolic)
                {
                    ...
                    if (ioiAddress[i] == '.')
                    {
                        ...
                        if (string.IsNullOrEmpty(currentValue) && lastAddedChar != '\0')
                            return null;
                        retVal.Add(new IOISegment()
                        {
                            SegmentType = IOISegmentType.Symbolic, SegmentString = currentValue
                        });
                    ...
                    else if (ioiAddress[i] == '[')
                    {
                        ...
                        currentValue = "";
                        currentSegmentType = IOISegmentType.Element; //AK
                        //currentSegmentType = IOISegmentType.None; //AK
                        lastAddedChar = '\0';
                    }
                    else
                    {
                ...
                else if (currentSegmentType == IOISegmentType.Element)
                {
                    ...
                    {
                        retVal.Add(
                            new IOISegment() {SegmentType = IOISegmentType.Element, SegmentString = currentValue});
        ...
        private static byte[] BuildSymbolicSegment(string stringSegment)
        {
            ...
            int arrayLen = len + (len % 2) + 2; // + 2 //Add pad if necessary
