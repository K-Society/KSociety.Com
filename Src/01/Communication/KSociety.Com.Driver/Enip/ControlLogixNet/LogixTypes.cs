...
namespace KSociety.Com.Driver.Enip.ControlLogixNet
{
    ...
    public static class LogixTagFactory
    {
        ...
        internal static LogixTag CreateQuickTag(string Address, LogixProcessor Processor, ushort Elements)
        {
            ...
            else
            {
                ...
                switch ((CipType)ti.DataType)
                {
                    case CipType.BITS:
                    case CipType.BOOL:
                        at = new LogixBOOL(Address, Processor, Elements);
                        break;
                    case CipType.SINT:
                        at = new LogixSINT(Address, Processor, Elements);
                        break;
                    case CipType.INT:
                        at = new LogixINT(Address, Processor, Elements);
                        break;
                    case CipType.DINT:
                        at = new LogixDINT(Address, Processor, Elements);
                        break;
                    case CipType.LINT:
                        at = new LogixLINT(Address, Processor, Elements);
                        break;
                    case CipType.REAL:
                        at = new LogixREAL(Address, Processor, Elements);
                        break;
                    default:
                        break; //Unknown type...
                }
        ...
        private static TemplateInfo ReadStructAttributes(ushort structureId, LogixProcessor processor)
        {
            ...
            ucmm.MessageRequest.Request_Path = new byte[]
            {
                0x20, 0x6C, 0x25, 0x00, (byte)((structureId & 0x00FF)), (byte)((structureId & 0xFF00) >> 8)
            };
        ...
        private static LogixTag CreateStructTag(string Address, LogixProcessor Processor, ushort Elements,
    ...
    internal class UDTItem
    {
        ...
        public object GetMemberValue(string name)
        {
            ...
            {
                case CipType.BITS:
                    return TypeConverter.GetBoolArray(
                        TypeConverter.GetInt32Array(_data, mi.MemberOffset, mi.MemberSize));
                case CipType.BOOL:
