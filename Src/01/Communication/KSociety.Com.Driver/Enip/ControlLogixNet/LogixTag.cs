using System;
using KSociety.Com.Driver.Enip.ICommon; //using Std.Com.Driver.Properties;
...
namespace KSociety.Com.Driver.Enip.ControlLogixNet
{
    ...
    public abstract class LogixTag : ITag
    {
        ...
        ///<summary>Gets the <see cref="LogixTag"/> that this <see cref="ControlLogixNET"/> belongs to.</summary>
        ///<remarks>
        ///<para>This provides a reference from the <see cref="LogixProcessor"/> to the <see cref="LogixTag"/> that the tag belongs to. This
        ///property is automatically set when a tag is added to a processor. By design, any <see cref="LogixProcessor"/> can only belong to a single
        ///<see cref="LogixTag"/> at any time. Adding a <see cref="LogixProcessor"/> to another <see cref="LogixProcessor"/> will result in the
        ///tag being removed from one <see cref="LogixTag"/> before being added to the other. </para>
        ///</remarks>
        ///<example>
        ///This example shows how to get the processor from a <see cref="LogixProcessor"/>.
        ///<code lang="cs">
        ///public static class Program
        ///{
        ///     public static void Main(string[] args)
        ...
        ///<summary>Gets or Sets any symbol string to be associated with the tag.</summary>
        ///<remarks>This property is available to the user to store symbolinc name. The data stays with the tag
        ///in the system and is not modified outside of user code.</remarks>
        ///<example>
        ///This example shows how to store and retrieve symbol from the <see cref="Symbol"/>.
        ///<code lang="cs">
        ///public static class Program
        ///{
        ///     public static void Main(string[] args)
        ...
        ///         LogixTag myTag = new LogixTag("sample");
        ///
        ///         myTag.Symbol = "Stored string";
        ///         string storedString = myTag.Symbol;
        ///
        ///     }
        ///}
        ///</code>
        ...
        /// </summary>
        public abstract LogixTypes LogixType { get; }
        ...
        internal void SetTagError(byte[] value)
        {
            ...
            switch (value[0])
            {
                ...
                case 0xFF:
                    if (value.Length != 4)
                    {
                        _lastError = ErrorStrings.Unknown;
                    ...
                    else
                    {
                        ...
                        {
                            case 0x05:
                                _lastError = ErrorStrings.AccessBeyondObject;
                                _lastErrorNumber = (int)LogixErrors.AccessBeyondObject;
                                _tagQuality = TagQuality.Bad;
                                break;
                            case 0x07:
                                _lastError = ErrorStrings.AbbreviatedTypeError;
                                _lastErrorNumber = (int)LogixErrors.AbbreviatedTypeError;
                                _tagQuality = TagQuality.Bad;
                                break;
                            case 0x04:
                                _lastError = ErrorStrings.BeginOffsetError;
...
#endif
        {
            ...
            else
            {
                ...
                {
                    _tagQuality = TagQuality.Bad;
                    _lastError = ErrorStrings.TypeMismatch + "0x" + typeCode.ToString("X2");
                    _lastErrorNumber = (int)LogixErrors.TypeMismatch;
                    throw new Exception(ErrorStrings.TypeMismatch + "0x" + typeCode.ToString("X2"));
                }
            }
